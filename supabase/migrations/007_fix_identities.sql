-- ============================================================
-- 修复版 V5 - 修复 auth.identities 表缺失问题
-- 这是导致工号登录失败的真正原因！
-- ============================================================
...
-- ============================================================
-- 辅助函数：判断当前用户是否为管理员（只查 auth.users，避免递归）
-- ============================================================
...
AS $$
BEGIN
  RETURN EXISTS (SELECT 1 FROM auth.users WHERE id = auth.uid() AND email = 'admin@admin.com');
END;
$$;
...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    v_user_id, v_email, v_encrypted_pw,
    now(), now(), now(),
  ...
  FOR v_idx IN 0..jsonb_array_length(p_users) - 1 LOOP
    ...
    BEGIN
      ...
      INSERT INTO auth.users (
        ...
        v_user_id, v_email, public.supabase_crypt(v_password),
        now(), now(), now(),
